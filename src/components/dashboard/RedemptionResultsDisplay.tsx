...
interface RedemptionResultsDisplayProps {
...
export function RedemptionResultsDisplay({ results, formatCurrency, formatNumber }: RedemptionResultsDisplayProps) {
  if (!results) return <p className="text-muted-foreground">No redemption details to display.</p>;
  ...
  return (
    <div className="space-y-6 mt-4">
      <Card>
        <CardHeader>
          <CardTitle className="font-headline text-xl text-primary flex items-center">
            <Gem className="mr-2 h-5 w-5" /> Redemption Calculation Summary
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          <DetailRow label="Your Scheme Gold Value (at redemption)" value={formatCurrency(results.yourGoldValue)} />
          <DetailRow label="Total Jewelry Weight Redeemed" value={`${formatNumber(results.inputIntendedJewelleryWeight, 3)} g`} />
          {results.additionalGoldGrams >= 0 ? (
            <DetailRow
              label="Additional Gold Needed for Jewellery"
            ...
            />
             ...
             <DetailRow
              label="Surplus Gold Value (after jewellery)"
      ...
      <Card>
        <CardHeader>
          <CardTitle className="font-headline text-xl text-primary flex items-center">
            <TrendingUp className="mr-2 h-5 w-5" /> Gold Analysis (for this Redemption)
          </CardTitle>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-3 bg-primary/5 rounded-md">
            <p className="text-sm font-medium text-primary">Your Scheme Gold</p>
            <p className="text-lg font-semibold">{formatNumber(results.goldAnalysisYouHaveGrams, 3)} g</p>
            <p className="text-xs text-muted-foreground">Worth {formatCurrency(results.goldAnalysisYouHaveWorth)} (at redemption rate)</p>
          </div>
          <div className={`p-3 rounded-md ${results.goldAnalysisNeedAdditionalGrams > 0 ? 'bg-amber-500/10 dark:bg-amber-700/20' : 'bg-green-500/10 dark:bg-green-700/20'}`}>
            <p className={`text-sm font-medium ${results.goldAnalysisNeedAdditionalGrams > 0 ? 'text-amber-700 dark:text-amber-400' : 'text-green-700 dark:text-green-400'}`}>
              {results.goldAnalysisNeedAdditionalGrams > 0 ? 'Need Additional for Jewellery' : (results.goldAnalysisNeedAdditionalGrams === 0 ? 'Exact Amount for Jewellery' : 'Surplus Gold after Jewellery')}
            </p>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="font-headline text-xl text-primary flex items-center">
            <FileText className="mr-2 h-5 w-5" /> Complete Invoice Breakdown (Redemption)
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="p-4 bg-green-50 dark:bg-green-900/30 rounded-md border border-green-200 dark:border-green-700">
            ...
            <DetailRow label="Your Scheme Gold Value Deduction" value={`-${formatCurrency(results.invoiceGoldValueDeduction)}`} />
            <DetailRow
           ...
           <div className="text-center pt-2">
            <p className="text-sm text-muted-foreground">Final Amount to Pay for Jewellery</p>
      ...
      {results.finalAmountToPay > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="font-headline text-xl text-primary flex items-center">
              <ListChecks className="mr-2 h-5 w-5" /> Final Payment Breakdown (Redemption)
            </CardTitle>
          </CardHeader>
       ...
       <Card>
        <CardHeader>
            <CardTitle className="font-headline text-xl text-primary flex items-center">
                <BookOpen className="mr-2 h-5 w-5" /> How Redemption Calculation Works
            </CardTitle>
        </CardHeader>
        <CardContent>
